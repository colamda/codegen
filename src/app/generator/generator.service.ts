...
export class GeneratorService {
  ...
  private readConfigFile(configFilePath: string): CodegenConfig {
    ...
    codegenConfig.modelConfigs = codegenConfig
      .modelConfigs
      .filter(config => {
    ...
    // -- adjust wipe paths
    codegenConfig.wipeDirectories = codegenConfig
      .wipeDirectories
      .map(wipePath => FileUtils.getAbsolutePath(configFileDir, wipePath));
   ...
   */
  private makeDestinationPathAbsolute(configFilePath: string, config: CodegenModelConfiguration): CodegenModelConfiguration {
    config.destination = FileUtils.getAbsolutePath(configFilePath, config.destination);
    return config;
  }
  ...
  private testProcessorMatches(mp: ModelProcessor, targets: string[]): boolean {
    ...
    return LangUtils.isDefined(targets.find(target => this.matches(mp.type, target)));
  }
